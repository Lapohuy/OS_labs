...
void* p1_thread(void* master_struct)
{
    ...
    while (step < info->round_num)
    {
        ...
        info->p1_own_state = CLIENT_STATE_WAIT_TURN;
        info->p2_own_state = CLIENT_STATE_MAKE_TURN;
        ...
        if (write(info->p2_tcp_socket_fd, &info->p2_own_state, sizeof(client_states)) <= 0)
            throw_exception_and_exit(info->p2_tcp_socket_fd, PLAYER_ONE);
...
void* p2_thread(void* master_struct)
{
    ...
    while (step < info->round_num)
    {
        ...
        info->p2_own_state = info->p1_own_state = CLIENT_STATE_WAITING_FOR_MASTER;

        if (write(info->p1_tcp_socket_fd, &info->p1_own_state, sizeof(client_states)) <= 0)
            throw_exception_and_exit(info->p1_tcp_socket_fd, PLAYER_TWO);
        ...
        sem_set_state(info->sem_id, MASTER, UNLOCK);
    }
...
void* m_thread(void* master_struct)
{
    ...
    while (step < info->round_num)
    {
        sem_set_state(info->sem_id, MASTER, LOCK);
        ...
        if (write(info->p1_tcp_socket_fd, &info->p1_own_state, sizeof(client_states)) <= 0)
            throw_exception_and_exit(info->p1_tcp_socket_fd, MASTER);
        
        if (write(info->p2_tcp_socket_fd, &info->p2_own_state, sizeof(client_states)) <= 0)
            throw_exception_and_exit(info->p2_tcp_socket_fd, MASTER);
        ...
        {
            info->p1_own_state = CLIENT_STATE_MAKE_TURN;
            info->p2_own_state = CLIENT_STATE_WAIT_TURN;
        }
...
int main(int argc, char* argv[])
{
    ...
    info.sem_id = semget(IPC_PRIVATE, 2, 0600 | IPC_CREAT);
    if (info.sem_id < 0)
    {
